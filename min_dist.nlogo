...
to find-new-spot
  if ( color = red ) [
    let target min-one-of (patches with [group1-happy and not any? turtles-here] )
      [ abs( pxcor - [ xcor ] of myself)  + abs ( pycor - [ ycor ] of myself ) ] ;; find a target for group1 without turtle above it and having nearest distance
    if ( target != nobody ) [
  ...
  if ( color = green ) [
    let target min-one-of( patches with [group2-happy and not any? turtles-here] )
       [ abs( pxcor - [ xcor ] of myself)  + abs ( pycor - [ ycor ] of myself ) ] ;; find a target for group2 without turtle above it and having nearest distance
      if ( target != nobody ) [
      set xcor [ pxcor ] of target
      set ycor [ pycor ] of target
    ]
...
to update-patches
  ask patches[
        ...
        ifelse nearby-total = 0 [    ;; if no turtle surrounded then potential happiness for group 1 and 2 is true
            set group1-happy true
            set group2-happy true
        ][
            set group1-happy ( nearby-group1 / nearby-total >= %-similar-wanted / 100)   ;; potential happiness false if  < similar wanted both for group1 and 2

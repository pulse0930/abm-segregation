...
to find-new-spot
  if ( color = red ) [
    let target min-one-of (patches with [group1-happy >= %-similar-wanted / 100 and not any? turtles-here]
      with-max [group1-happy])
      [  abs( pxcor - [ xcor ] of myself)  + abs ( pycor - [ ycor ] of myself )  ] ;; find a target for group1 without turtle above it and having the maximum happiness potential and having minimum distance
  ...
  if ( color = green ) [
    let target min-one-of( patches with [group2-happy >= %-similar-wanted / 100 and not any? turtles-here]
      with-max [group2-happy])
       [  abs( pxcor - [ xcor ] of myself)  + abs ( pycor - [ ycor ] of myself )  ]  ;; find a target for group2 without turtle above it and having the maximum happiness potential and having minimum distance
      if ( target != nobody ) [
      set xcor [ pxcor ] of target
      set ycor [ pycor ] of target
    ]

...
patches-own [
  group1-happy     ;; happiness potential for group 1
  group2-happy     ;; happiness potential for group 2
  nearby-group1    ;; how many turtle of group 1 surrounded?
  nearby-group2    ;; how many turtle of group 2 surrounded?
  nearby-total     ;; how many turtle of surrounded?
]
...
to go
  ...
  update-patches                ;; before movement compute potential happiness of patches
...
;;to find-new-spot
;;  if ( color = red ) [
;;    let target one-of (patches with [ group1-happy and not any? turtles-here])
;;    if ( target != nobody ) [
...
;;  ]
;;  if ( color = green ) [
;;    let target one-of( patches with [ group2-happy and not any? turtles-here])
;;      if ( target != nobody ) [
;;      set xcor [ pxcor ] of target
;;      set ycor [ pycor ] of target
;;    ]
...
;;to update-patches
;;  ask patches[
;;        set nearby-total count turtles-on neighbors
;;        set nearby-group1 count (turtles-on neighbors) with [ color = red ]
;;        set nearby-group2 count (turtles-on neighbors) with [ color = green]
;;        ifelse nearby-total = 0 [
;;            set group1-happy true
;;            set group2-happy true
;;        ][
;;            set group1-happy ( nearby-group1 / nearby-total >= %-similar-wanted/100)
;;            set group2-happy ( nearby-group2 / nearby-total >= %-similar-wanted/100)
;;        ]
;;  ]
;;end
...
;;to find-new-spot
;;  if ( color = red ) [
;;    let target one-of (patches with [not any? turtles-here] with-max [group1-happy]) ;; find a target for group1 without turtle above it and having the maximum happiness potential
;;    if ( target != nobody ) [                                                        ;; NOTE: some patches may have same happiness potential
;;      set xcor [ pxcor ] of target
;;      set ycor [ pycor ] of target
;;    ]
;;  ]
;;  if ( color = green ) [
;;    let target one-of( patches with [ not any? turtles-here] with-max [group2-happy]) ;; find a target for group2 without turtle above it and having the maximum happiness potential
;;      if ( target != nobody ) [
;;      set xcor [ pxcor ] of target
;;      set ycor [ pycor ] of target
;;    ]
...
to find-new-spot
  if ( color = red ) [
    let target max-one-of (patches with [group1-happy >= %-similar-wanted / 100 and not any? turtles-here]) [group1-happy] ;; find a target for group1 without turtle above it and having the maximum happiness potential
    if ( target != nobody ) [                                                        ;; NOTE: some patches may have same happiness potential
      set xcor [ pxcor ] of target
      set ycor [ pycor ] of target
    ]
  ...
  if ( color = green ) [
    let target max-one-of (patches with [group2-happy >= %-similar-wanted / 100 and not any? turtles-here]) [group2-happy] ;; find a target for group2 without turtle above it and having the maximum happiness potential
      if ( target != nobody ) [
      set xcor [ pxcor ] of target
      set ycor [ pycor ] of target
    ]
...
to update-patches
  ask patches[
        ...
        set nearby-group1 count (turtles-on neighbors) with [ color = red ]
        set nearby-group2 count (turtles-on neighbors) with [ color = green]
        ifelse nearby-total = 0 [    ;; if no turtle surrounded then potential happiness for group 1 and 2 is 1 (maximum)
            set group1-happy 1
            set group2-happy 1
        ][                           ;; else calculate potential happiness for group 1 and 2
            set group1-happy ( nearby-group1 / nearby-total)   ;;NOTE: potential happiness will be 0 for anyone of the group both not both in this case..
            set group2-happy ( nearby-group2 / nearby-total)
        ]
  ]
end
